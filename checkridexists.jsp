...
try
{
	...
	rs = statement.executeQuery("SELECT 1 FROM v1x.companies_wait WHERE rid LIKE '" + em + "' and isenabled=1");
	if(rs.next()) {
	 	r1 = new JSONObject();
       	r1.put("EX", "1");
      	r.add(r1);		
	...
	{
		rs.close(); rs=null;
		rs = statement.executeQuery("SELECT 1 FROM v1x.companies_approved WHERE rid LIKE '" + em + "' and isenabled=1");
		if(rs.next()) {
		 	r1 = new JSONObject();
	       	r1.put("EX", "2");
	      		r.add(r1);		
